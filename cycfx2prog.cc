...
int cycfx2progmain(struct usb_device *usbdev, const char *hexfile, const char *cmd)
{
	if (usbdev == NULL || cycfx2.open(usbdev))
	{
		return(1);
	}

	int errors = 0;
	DPRINT("Putting 8051 into reset.\n");
	errors += cycfx2.FX2Reset(/*running=*/0);

	if (!hexfile)
	{
		DPRINT("Command \"dl\" requires file to download.\n");
		++errors;
	}
	else
	{
		DPRINT("Programming 8051 using \"%s\".\n", hexfile);
		errors += cycfx2.ProgramIHexFile(hexfile);
	}

	DPRINT("Putting 8051 out of reset.\n");
	errors += cycfx2.FX2Reset(/*running=*/1);
	errors += cycfx2.close();
	DPRINT("cycfx2prog: %s!", errors ? "error!" : "successfully!");

	return(errors ? 1 : 0);
}

int cymain(int argc, char **arg)
{
	int errors=0;
	...
	for(int i=1; i<argc; i++)
	{
		...
		else if(!strncmp(arg[i],"-id=",4))
		{
			...
			if(rv<2)
			{  DPRINT("Illegal argument for option -id=\n");
				++errors;  }
			else if(rv==2)
			{  arg_nth=0;  }
			else if(arg_nth<0)
	...
	if(rv<0)
	{  DPRINT("usb_find_busses failed (rv=%d)\n",rv);  return(1);  }
	rv=usb_find_devices();
	if(rv<0)
	{  DPRINT("usb_find_devices failed (rv=%d)\n",rv);  return(1);  }
	...
	if(arg_bus_dev)
	{
		...
		if(!usbdev)
		{  DPRINT("Illegal/nonexistant device %s.\n",arg_bus_dev);
			return(1);  }
	}
	else
	{
		...
		{
			if(arg_id_specified)
			{  DPRINT("Device with vendorID=0x%04x, productID="
				"0x%04x, nth=%d not attached.\n",arg_vend,arg_prod,arg_nth);  }
			else
			{  DPRINT("No unconfigured Cypress FX2 attached.\n");  }
			return(1);
		}
	}
	
	DPRINT("Using ID %04x:%04x on %s.%s.\n",
	...
	for(int argc_i=1; argc_i<argc; argc_i++)
	{
		...
		if(first_arg)
		{
			do {
				*(first_arg++)='\0';
				if(nargs>=MAXARGS)
				{
					DPRINT("Too many arguments for command \"%s\" "
		...
		else if(!strcmp(cmd,"dram"))
		{
			...
			DPRINT("Dumping %u bytes of RAM at 0x%x:\n",len,adr);
		...
		else if(!strcmp(cmd,"set"))
		{
			...
			{  DPRINT("Command set: Illegal/missing address "
				"and/or value.\n");  ++errors;  }
			else
			{
				DPRINT("Setting value at 0x%x to 0x%x\n",adr,val);
		...
		else if(!strcmp(cmd,"dbulk"))
		{
			...
			{  DPRINT("Command dbulk: Illegal/missing "
			...
			else
			{
				...
				for(int i=0; i<num; i++)
				{
					DPRINT("Reading %s%d bytes from EP adr 0x%02x\n",
						type=='B' ? "<=" : "",len,ep);
		...
		else if(!strcmp(cmd,"sbulk"))
		{
			...
			if(a[0] && *a[0])  {  ep=strtol(a[0],NULL,0);  }
			if(ep<0 || ep>=127)
			{  DPRINT("Command sbulk: Illegal/missing "
				"endpoint.\n");  ++errors;  }
			else
			{
				DPRINT("Sending %d bytes to EP adr 0x%02x\n",len,ep);
		...
		else if(!strcmp(cmd,"fbulk"))
		{
			...
			do {
				...
				DPRINT("Sending \"%s\" in chunks of %d bytes to EP adr "
					"0x%02x\n",
				...
				DPRINT("Sent %d bytes to EP adr 0x%02x\n",tot_bytes,ep);
			} while(0);
		}
		else if(!strcmp(cmd,"bench_bulk"))
		{
			...
			{  DPRINT("Command bench_bulk: Illegal/missing "
				"endpoint/length/number.\n");  ++errors;  }
			else
			{
				// IN endpoints have the bit 7 set. 
		...
		else if(!strcmp(cmd,"ctrl"))
		{
			...
			DPRINT("Sending control message type 0x%02x, request "
		...
		{
			DPRINT("Ignoring unknown command \"%s\".\n",cmd);
			++errors;
		}
