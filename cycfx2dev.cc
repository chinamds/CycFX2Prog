	...
	char type)
{
	...
	if(!IsOpen())
	{  DPRINT("BlockRead: Not connected!\n");  return(-1);  }
	...
	{  DPRINT("Failed to claim interface %d: %s\n",
	...
	do {
		...
		{
			DPRINT("Failed to set altinterface %d: %s\n",
		...
		while(left)
		{
			...
			if(rv<0)
			{
				DPRINT("Reading %zu bytes from EP 0x%02x: "
					"USB: %s SYS: %s\n",
			...
			if((size_t)rv<bs)
			{
				...
				//if(!rv)
					DPRINT("Short read (%zd/%zu bytes)%s\n",rv,bs,
						rv==0 ? " (timeout?)" : "");
				if(rv==0)
				{  ++error;  goto breakout;  }
			}
	...
	size_t nbytes,char type)
{
	...
	if(!IsOpen())
	{  DPRINT("BlockWrite: Not connected!\n");  return(-1);  }
	...
	do {
		...
		while(left)
		{
			...
			if(rv<0)
			{
				DPRINT("Writing %zu bytes to EP 0x%02x: "
					"USB: %s SYS: %s\n",
					bs,endpoint,usb_strerror(),strerror(-rv));
				++error;
				goto breakout;
			...
			if((size_t)rv<bs)
			{
				// Not sure if rv=0 is a valid timeout indication...
				//if(!rv)
					DPRINT("Short write (%zd/%zu bytes)%s\n",rv,bs,
						rv==0 ? " (timeout?)" : "");
				if(rv==0)
				{  ++error;  goto breakout;  }
			}
...
{
	if(!IsOpen())
	{  DPRINT("BenchBlockRead: Not connected!\n");  return(1);  }
	...
	size_t nbytes)
{
	...
	while(d<dend)
	{
		...
		int rv=usb_control_msg(usbhdl,0x40,0xa0,
			...
			/*timeout=*/1000/*msec*/);
		if(rv<0)
		{  DPRINT("Writing %zu bytes at 0x%zx: %s\n",
...
int CypressFX2Device::ReadRAM(size_t addr,unsigned char *data,size_t nbytes)
{
	...
	while(d<dend)
	{
		...
		int rv=usb_control_msg(usbhdl,0xc0,0xa0,
			...
			/*timeout=*/1000/*msec*/);
		if(rv<0)
		{  DPRINT("Reading %zu bytes at 0x%zx: %s\n",
	...
	const char *path,int line)
{
	...
	if(type==0)
	{
		...
		{  DPRINT("%s:%d: format violation (4)\n",path,line);
			return(1);  }
		if((cksum+file_cksum)&0xff)
		{  DPRINT("%s:%d: checksum mismatch (%u/%u)\n",
	...
	{
		DPRINT("%s:%d: Unknown entry type %d\n",path,line,type);
		return(1);
	}
...
{
	if(!IsOpen())
	{  DPRINT("ProgramIHexFile: Not connected!\n");  return(1);  }
	...
	const unsigned char *ctl_buf,size_t ctl_buf_size)
{
	...
	int rv=usb_control_msg(usbhdl,requesttype,request,
		...
		/*timeout=*/1000/*msec*/);
	if(rv<0)
	{  DPRINT("Sending USB control message: %s\n",
...
int CypressFX2Device::open(struct usb_device *_usbdev)
{
	...
	if(!usbhdl)
	{  DPRINT("Failed to open device: %s\n",usb_strerror());
		return(1);  }
	return(0);
}
